...
/* --------------------------------------------------------------
   ...
   Phase M4-S.1 – architecture foundation
   Provider: Oli
   --------------------------------------------------------------
*/
...
class OliLetterConfiguratorModuleCenterModule extends AbstractModuleCenterModule
{
    ...
    public function install(): void
    {
        ...
        $this->writeSetting('module_version', self::MODULE_VERSION);
        $this->writeSetting('schema_version', self::SCHEMA_VERSION);
        $this->writeSetting('configuration_schema_version', '1');
    }
    ...
    public function uninstall(): void
    {
        $tables = [
            ...
            'oli_lc_product_template_thicknesses',
    ...
    {
        $this->db->query(
            "INSERT INTO `oli_lc_settings` (`setting_key`, `setting_value`) VALUES (" .
    ...
    /**
     * Keeps older installations compatible with the current module skeleton.
     * Only additive, module-owned schema changes are applied here.
     */
    private function ensureCurrentSchema(): void
    {
        $this->addColumnIfMissing(
            'oli_lc_product_templates',
            'thickness_mode',
            "`thickness_mode` ENUM('all','selected') NOT NULL DEFAULT 'all' AFTER `color_mode`"
        );
        $this->addColumnIfMissing(
            'oli_lc_product_templates',
            'price_profile_id',
            "`price_profile_id` INT UNSIGNED NULL AFTER `thickness_mode`"
        );
        $this->addColumnIfMissing(
            'oli_lc_thicknesses',
            'production_method',
            "`production_method` VARCHAR(32) NOT NULL DEFAULT 'cnc_fraesen' AFTER `material_id`"
        );
        $this->addColumnIfMissing(
            'oli_lc_thicknesses',
            'thickness_min_mm',
            "`thickness_min_mm` DECIMAL(10,3) NULL AFTER `thickness_mm`"
        );
        $this->addColumnIfMissing(
            'oli_lc_thicknesses',
            'thickness_max_mm',
            "`thickness_max_mm` DECIMAL(10,3) NULL AFTER `thickness_min_mm`"
        );

        $this->db->query(
            "UPDATE `oli_lc_thicknesses` SET `thickness_min_mm` = `thickness_mm` " .
            "WHERE `thickness_min_mm` IS NULL"
        );
        $this->db->query(
            "UPDATE `oli_lc_thicknesses` SET `thickness_max_mm` = `thickness_mm` " .
            "WHERE `thickness_max_mm` IS NULL"
        );
    }
    ...
    private function addColumnIfMissing(string $table, string $column, string $definition): void
    {
        ...
        if ($result->num_rows === 0) {
            $this->db->query("ALTER TABLE `" . $safeTable . "` ADD " . $definition);
        }
    }
    ...
    private function getCreateTableStatements(): array
    {
        return [
            ...
            "CREATE TABLE IF NOT EXISTS `oli_lc_product_templates` (
                ...
                `thickness_mode` ENUM('all','selected') NOT NULL DEFAULT 'all',
                `price_profile_id` INT UNSIGNED NULL,
                `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
                `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
            ...
            "CREATE TABLE IF NOT EXISTS `oli_lc_production_methods` (
                `production_method_id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
                `name` VARCHAR(120) NOT NULL,
                `method_key` VARCHAR(64) NOT NULL,
                `engine_key` VARCHAR(32) NOT NULL,
                `range_mode` ENUM('single','range') NOT NULL DEFAULT 'single',
                `machine_name` VARCHAR(160) NOT NULL DEFAULT '',
                `machine_cost_per_hour` DECIMAL(12,4) NOT NULL DEFAULT 0.0000,
                `is_active` TINYINT(1) NOT NULL DEFAULT 1,
                `sort_order` INT NOT NULL DEFAULT 0,
                `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
                `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
                PRIMARY KEY (`production_method_id`),
                UNIQUE KEY `uq_oli_lc_method_key` (`method_key`)
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci",

            "CREATE TABLE IF NOT EXISTS `oli_lc_product_assignments` (
                ...
                UNIQUE KEY `uq_oli_lc_assignment_product` (`products_id`),
                KEY `idx_oli_lc_assignment_template` (`product_template_id`),
                KEY `idx_oli_lc_assignment_active` (`is_active`)
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci",
